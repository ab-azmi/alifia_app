<div class="w-full overflow-hidden"
    ...
    <div class="border-b flex flex-col overflow-y-scroll grow h-full">
        ...
        <header class="w-full sticky inset-x-0 flex pb-[5px] pt-[5px] top-0 z-10 bg-white border-b">
            <div class="flex w-full items-center px-2 lg:px-4 gap-2 md:gap-5">
                ...
                <h6 class="font-bold truncate">
                    {{ $selected_conversation?->getReceiver()->email }}
                </h6>
            </div>
        </header>
        ...
        <main
            ...
            @forelse ($loaded_messages as $msg)
            <div @class([ 'max-w-[80%] md:max-w-[78%] flex w-auto gap-2 mt-2' , 'ml-auto'=> $msg->sender_id ===
                ...
                <div @class([ 'flex flex-wrap text-[15px] rounded-xl p-2.5 flex-col text-black bg-[#f6f6f8fb]'
                    , 'rounded-bl-none border border-gray-200/40'=> !($msg->sender_id === auth()->id()),
                    'rounded-br-none bg-primary/80 text-white' => $msg->sender_id === auth()->id(),
                    ])>
                    <p class="whitespace-normal truncate text-sm md:text-base text-white tracking-wide lg:tracking-normal">
                        {{ $msg->body }}
                    </p>
